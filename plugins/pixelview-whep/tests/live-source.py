...
cmd=['clang','-Wall','-Wextra','-Werror','-Wno-unused-parameter','-I'+str(repo/'libobs'),'-I'+str(build/'config'),'-I'+str(deps/'include'),'-F'+str(framework),'-framework','libobs','-Wl,-rpath,'+str(framework),'-Wl,-rpath,'+str(deps/'lib'),*flags,str(root/'tests/live-source.c'),'-o',str(contents/'MacOS/live-source'),'-L'+str(stage/'lib'),'-lgstapp-1.0.0','-lgstvideo-1.0.0','-lgstaudio-1.0.0','-lgstreamer-1.0.0','-lgobject-2.0.0','-lglib-2.0.0','-Wl,-rpath,'+str(stage/'lib')]
