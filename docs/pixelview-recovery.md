...
Session-only stream intent is retained after transient control disconnect, connect timeout or ping silence, or native media disconnected/connect-failed completion. Recovery uses the native profile's `Output/Reconnect`, `RetryDelay`, and `MaxRetries` values, with the configured fixed delay and a consecutive-attempt counter reset only by actual media start. It drains the old output/setup, opens a new control connection, requests a fresh start, and only then resumes native WHIP. A control drop while media is already running is not a stream failure: media continues and the reconnected socket needs no new start. Raw libobs WHIP retry remains disabled because it would restart with the cached service, bypassing the authority gate.
