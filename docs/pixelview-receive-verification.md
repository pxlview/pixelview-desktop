...
- The latest session ID, edited receiver name and password now persist across app launches. ID/name/revision use user configuration; the password uses a dedicated device-only, non-synchronizable macOS Keychain item bound to the exact session/backend/revision, not sender pairing. This supersedes the earlier memory-only retention requirement. Passwords never enter plaintext config, scenes or logs.
- The masked password stays in form memory across Start/Stop/new Start and login failure. Session/password fields are locked while connecting/receiving and editable again after Stop/failure. Session-ID edits clear the form and stored password; deep links replace and persist both fields atomically without starting, including clearing a same-ID password when the token is absent. Name/session edits save without Start; passwords save on editingFinished/Start/link and on clean shutdown with a pending edit. Shutdown drops process copies, not the saved credential.
- Compiled production Qt UI/controller + real isolated Keychain regressions verify separate-process restart, edited name/password reuse, HTTP401 correction, exact session/origin binding, same-ID missing-token deletion, missing/corrupt Keychain entries, fault-injected secure-storage failures, real config-save failure, masking and config/log non-disclosure. Storage warnings do not replace login errors. Test-only Keychain cleanup is read back absent even on failure. Full-app build and GUI acceptance of this persistence change remain separate; see `pixelview-deep-links.md` for failure semantics.
...
- Exact unique backend viewer/name correlation; Stop removed that live record. The then-current UI cleared the password (superseded by the secure persistence policy above); mode tabs unlocked, Sending/Receiving roundtrip and new Start recovered media.
