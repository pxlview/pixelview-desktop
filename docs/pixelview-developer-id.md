...
The full Pixelview suite passes **90 tests**, including the new native SDK/deployment regression and the four signing-helper tests (plus existing disposable Keychain/loopback fixtures, not the real pairing account). `bash -n` and `git diff --check` passed.
