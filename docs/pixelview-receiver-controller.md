...
Transient transport/HTTP408/429/502/503/504 loss disconnects media before exponential1–30second reauthentication. Auth/TLS/protocol denial clears credentials and requires manual Start; no background restart on launch. A23-hour refresh reconnects before the backend's86000-second token lifetime. Status messages are fixed/sanitized; server error bodies and URLs never appear in UI status.

Credentials remain in memory for the active intent only; QString/NSURLSession memory is not claimed to provide cryptographic zeroization. The media source can independently fail even when control remains Ready; frontend must display source status separately and retain an operational Stop.
