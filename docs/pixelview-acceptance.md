...
- The existing Start/Stop button sends `DESKTOP_START` and creates the in-memory `whip_custom` service from `DESKTOP_STARTED`. Async native preparation rechecks start authority before starting. Native WHIP retries are disabled; control reconnect backs off from one to thirty seconds. A control drop while streaming keeps the media running and reconnects without a new start. Actual native output stop is verified before sending `DESKTOP_STOP` or closing the control connection. Liveness is the server's 20-second ping answered by `PONG_RESPONSE`; 60 seconds without a ping is treated as a dead socket.
- Pongs report real OBS output dimensions/FPS, selected encoder classification, bitrate in bits/sec, profile and an allowlist of nonsecret native options. Arbitrary encoder option strings are not transmitted. Admin `streaming` remains a client output-state report, not independent media observation.
- Missing WHIP/v2 config is an explicit error; SRT is never used. Inactive project, paused node, subscription and other protocol failures fail closed. `SOCKET_DESKTOP_REVOKED` (or a 4401 upgrade rejection) discards the process-held device credential and requires re-pairing; `SOCKET_DESKTOP_REPLACED` stops reconnecting without stopping a running stream.
...
Current scope includes macOS Desktop pairing, authenticated backend WebSocket and native WHIP started through the control socket. The historical checks below predate that integration; receiver/DeckLink playout is not implemented here.
